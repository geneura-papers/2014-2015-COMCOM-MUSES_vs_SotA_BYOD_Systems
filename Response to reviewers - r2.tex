...
% Antonio - Voy a sustituir el Verbatim por un cambio de color en las respuestas porque al compilarlo no hace los saltos de lÌnea si no se los especificas y el texto se sale del margen.
...
\textcolor{blue}{We would like to thank the reviewer for his/her opinion and would lead him/her to have another look at the new version, since it has been highly improved. 
As a summary:}
%
\textcolor{blue}{
\begin{itemize}
...
\item The description of MUSES system is now much better. Several points have been clarified and better explained in Section 4. Moreover, a prototype of the system has been tested in an actual company with success, according to the obtained results. This is also commented by the end of that section.
...
\item A complete proof-editing process has been done, revising the English grammar and rewriting several paragraphs for a better comprehension.
\end{itemize}}
% Antonio - TODO: AÒadir m·s cosas (medianamente relevantes) que se hayan hecho
%
\textcolor{blue}{These changes are better described in the responses to the others reviewers, so we encourage this reviewer to read them to have a better idea of their impact in the new version of the manuscript.}
...
\textcolor{blue}{MUSES starts from a defined set of security rules that the CSO must define. Ideally, these rules should deal, in principle, all the potential security incidents that the company aims to detect. MUSES would be able to define new rules (by means of refinement) which could be labelled (i.e. assigned a decision) after the corresponding events have happened. However, it can also infer or create new rules, which the CSO must approve, using computational intelligence techniques. These rules could deal with expected situations (not previously happened).
...
\textcolor{blue}{Moreover, MUSES offers visual analysis tools to the CSOs, which can aid (or guide) they to detect suspicious or anomalous patterns, and to study how to avoid these patterns become malicious to the company.}
...
% Antonio - yo pienso que en este sistema cualquier violaciÛn de seguridad que se detecte se har· por medio de las reglas de seguridad. Eso serÌa lo ideal, pero para ello el CSO debe definir un conjunto que asegure la seguridad del sistema en la mayorÌa de casos. Luego el KRS podr· proponer nuevas reglas a travÈs de 'predicciones' (inferencias) o refinamiento, pero estas ˙ltimas se construir·n como reglas optimizadas para tratar con incidentes de seguridad ya detectados. No se pueden crear de la nada reglas para tratar con nuevos incidentes (desconocidos).
...
% Antonio - TODO: aclarar lo otro que comento en la respuesta (la idea de empezar con reglas restrictivas e irlas refinando) en el texto.
...
Reviewer is right. Although several works have been cited in other sections of the work, a proper discussion on research on BYOD was necessary. Therefore, we have analysed the 3 papers suggested by the reviewer, together with several new relevant ones. These new references also have been used along other sections of the work to justify MUSES features (for example, the set of best practices used) and to propose the new taxonomy, taking into account the works described in the new section. The new references added \cite{Garba15organisational,Scarfo12survey,Gessner13userfriendly,Armando14metamarket,Haejung12Door,Samaras13SaaS,Miller12Privacy,Romer14BestPractices} can be found at the end of this document.
...
This section has been rewrited, and the references mentioned by the referee (including new products) have been added. %FERGU: PONED ESTO M√ÅS BONICO!
...
% Antonio - TODO: Incluir justificaciÛn de Big Data Processing considerando el n˙mero de eventos generados en las Trials y extrapol·ndolo a muchos meses (al menos los 6 del hard limit).
...
% Antonio - no sÈ a quÈ 'bullets' se refiere
...
For the section 5 and discussion, this reviewer would suggest to consider providing overview in the form of table(s) or figure, comparing different aspects of MUSES and other tools and providing hints/references to further details in this or other works. As there are many features of MUSES and other tools, employed approaches and techniques, different coverages and limitations, such overview could contribute to clarity and position of the proposed solution.

\textcolor{blue}{We agree with the reviewer. A comparative table has been included in the section 5 (Table 1 - Page XX), summarizing some of the features of the presented systems and those of MUSES system.}

% Antonio - TODO: M·s Tablas comparativas. øAntares, puedes hacerlas?
% Antonio - øquiz· una tabla con lo de la taxonomÌa y en quÈ categorÌa entra cada sistema?
...
\textcolor{blue}{We think that the work has been highly improved following the reviewer's suggestions, and would like to thank you for your careful revision and useful comments.}
