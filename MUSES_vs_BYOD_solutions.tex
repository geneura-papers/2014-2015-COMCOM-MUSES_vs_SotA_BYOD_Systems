...
In this scenario several solutions have appeared in order to manage corporate security. Most of the solutions are focused for both smartphones and desktop PCs, although some are implemented for a certain platform. However, almost all of them try to be non-intrusive (regarding the users' personal data), friendly and easy to use. 

Moreover, it has been demonstrated that employees who ignore the company security policies are the main hazard for the company security \cite{Adams_Users05}. In this situation, the tools that are appearing in the market aim to cope with the concept of seamless working experience on different devices. This concept is a methodology of work which allows users to start/continue a working session, over multiple devices and locations, without any significant loss of data. This new situation has a big impact from the point of view of the security \cite{Schu_SecPatterns05}, since the company data borders have changed in the last years, ans therefore now the users can access important data from outside the enterprise, and possibly through a non absolutely secure channel. %Antares - Yo quitaba lo de significant loss of data, ya que significant se repite después.
%Cambiado
...
Security Policies. In addition, it has the ability of `learning' from the user's (the employees)
past behaviour and adapt the set of rules, or even inferring new ones,
in order to deal with potential security incidents that they might cause in the future. %FERGU: The user's "algo que falta"? o eso hace referencia a
        %su culpa? 
%"lo del usuario", literalmente. Pero mejorquitarlo - JJ
Then the system will react, in a non-intrusive way, to the potentially
dangerous sequence of actions (events) that the employees are conducting at
any time. 
...
submitted. After the release of the first prototype, an additional
work on the data mining techniques was conducted in \cite{mora14:urls} so that we have a
clearer idea of MUSES capabilities and, as such, they will be presented in
this paper. 

MUSES will include some important modules in it core, %FERGU: no habéis hablado de ese loop, decid mejor "core" (no lo cambio yo por si no tiene sentido)
%cambiado
