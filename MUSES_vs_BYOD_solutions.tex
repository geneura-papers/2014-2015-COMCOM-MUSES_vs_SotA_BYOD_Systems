...
\begin{description}
  ...
  \item[DEVICE PROTECTION SENSOR] Which scans device security settings, such as the time until the device screen turns off, if the device has the screen protected with a password, or even if the device is rooted \cite{hildenbrand2014}. This sensor %¿cambiarlo por "This security mechanism" ?
   adds the possibility of detecting security violations event if the user is not doing a specific action.
...
checks if the device has already an antivirus, and raises a security violation alert if not. It might happen then that the user ignores this advice and the device becomes compromised. As it would be explained in the next section, the sensors are constantly sending their monitored values, and therefore patterns are created, stored, and processed on the server. In case the device gets infected with rootkit-type malware \cite{bickford2010rootkits}, a sudden change in the values of the sensors will be noticed by the server, which will immediately warn the user, and decrease the devide trust value.
