...
This system, which has released its first prototypes this year, is compared with the most relevant
...
Thus, this work introduces a recently released (in alpha) system named
...
Security Policies, but with the ability of `learning' from the user's
past behaviour and adapt the set of rules, or even inferring new ones,
in order to deal with potential future security incidents due to the
user's. %FERGU: The user's "algo que falta"? o eso hace referencia a
        %su culpa? 
%"lo del usuario", literalmente. Pero mejorquitarlo - JJ
Then the system will react, in a non-intrusive way, to the potentially
...
submitted. After the release of the first prototypes and additional
work on the data mining techniques used \cite{mora14:urls} we have a
clearer idea of MUSES capabilities and, as such, will be presented in
this paper. 
...
interacts with the devices, own or corporate, through the MUSES
graphical interface inside his or her own context (situation,
connection, status).
