...
can implemented in a secure way and comply with the Security Policies
mentioned above. In this paper we present an overview of BYOD security
solutions along a novel and open source system named
...
predict future security incidences produced by these users. % <-
                                % Solution, details. 
MUSES, which has released its first prototype in early 2015, is
...
As these tools have several access levels, different detection methods or data control, there exist a number of aspects that the end-user needs to take into account to validate if a product fits his needs. Therefore, we propose the next taxonomy in order to classify the BYOD tools described in this paper.
...
\item {\em Updatable of fixed information database}. This classification is related with the previous one. The new information gathered by the system during runtime can be stored in a database to generate new rules. If this database is not changed during runtime, as in the offline example explained above, the database is {\em fixed}. An example could be the hand-coded rules of a firewall, such as {\em iptables}, obtained from an offline method. On the other hand, an updatable database, such as the system logs, that can be used to extract new information, may offer more valuable source for dynamic adaptation mechanisms. However, it is not mandatory for an online threat detection method to use a updatable database, as all the current data may be analysed and deleted once a new rule are created. 
\item {\em Invasive or non-invasive data access}. A system can be considered {\em invasive} if a user needs to use the tool to perform his job. For example, open a specific browser provided by the tool, instead the original device browser, such as Chrome of Safari. A {\em non-invasive} data access is transparent to the user. An example is a log analyser daemon that updates other programs configuration or send information to the company server.
\item {\em Superuser or non-superuser system control}. Using a tool in superuser mode would add more information and control than a non-superuser execution, as it can access to elements of the device usually forbidden for regular applications, such as network, logs or other devices. However using superuser control may lead to a lack of privacy for the user.
\end{itemize}
...
The client is an important part of the a-priori risk treatment, because it both monitors the whole context and finally acts or warns the user in event of a security violation. Security violations are mainly identified by security rules, which is why MUSES uses DM and CI techniques to evolve them and enhance the coverage. That is to say, the more sensors are implemented, the more enforced the security in the device is. To implement a sensor means to include a programmed method which makes MUSES to start `listening' some parts of a device. 
...
The self-adaptation, to the user and context, of the set of Corporate Security Rules is one of the main features of this MUSES system.
To this end, the \textit{MusKRS} module is in charge of analysing all the gathered events, context, and user-related data. Then it processes the data and adapts or refines the security rules. This refinement is meant as an improvement of the existing security rules, and it aim is to deal with new, possibly insecure, events. As a result, a set of optimisations and additions to the set of security rules are proposed to the Chief Security Officer (CSO) of the company.
...
The refinement, or adaptation, of the security rules can be made through techniques as simpler as generalisation or specialisation of rules, for instance, or by more complex CI methods such as Evolutionary Algorithms.
...
The set of data are composed by the so-called patterns, which consist of an event and all the related information to that event. This means that the context in which the event was is translated into a number of \textit{attributes} of the pattern. This process is mainly non-supervised. Moreover, as the datasets are huge, because a lot of attributes are extracted from the events so the more complete information is obtained, MUSES applies Big Data processing methods \cite{BigData_11}. %JUSTIFY (Paloma will do it)

The methods that MUSES applies over the data are:

\begin{itemize}

\item \textit{Pattern Mining} \cite{PatternMining_Han07}: 
By this process, the patterns are obtained from the `raw' data, as has been explained. These form what is called a dataset, and from it, the most frequent or anomalous patterns, can be extracted. The idea is that non-frequent patterns are potentially suspicious, and thus, could be of interest to be checked by the CSO, or to serve as a reference for the rule-refinement process.
...
It consists of extracting the most important featuresvariables from the data. Feature selection techniques have been proven \cite{liu1998feature} to preserve the performance of a dataset in terms of classification accuracies, while significantly increasing the processing time.
...
\item User-related information, and context information in general, corresponding to those anomalous or unlabelled events. Thus, the user's location zone, role, or device settings, for instance, can be considered in order to select the applicable set of rules for that conditions.
...
\item The information about the user behaviour with respect to system messages, wether the users acts in consequence or against MUSES advice, as well as user feedback. The user feedback has been measured in the MUSES trials by making interviews with the people who have used MUSES, but it can be also measured by pop-up messages, ``has this information been useful?''-type. All this information contribute to the inference of new rules or in adaptation, in order to deal with, for instance, users that repeatedly ignored warning messages.
...
The generation of new rules is done by considering the so-called \textit{dictionary}, i.e. a set of terms corresponding to all the possible inputs and actions in the system, which are antecedents and consequents \footnote{The antecedents are the conditions of a rule that have to be met to apply the consequents.} in the security rules to be inferred.
The evaluation of these rules is done by considering the stored log information concerning the parameters along with the actions and decisions made in every component in the system. Thus, it is possible to `simulate' the whole system behaviour when the new rule is included and get a value of its performance.
...
The evaluation of the whole set of security rules is done by considering the number of unlabelled patterns that are `covered' after the adjustments. Again, the rules at the end of this step are presented to the CSO as ``refined''. The CSO is the one who decides, according to the preformance simulation results, if an inferred rule, or the refined one, is the rule which is finally included in the system.

\item A \textit{GA optimisation} algorithm for adapting the assets' values. These are numerical representations of the importance of the corporate assets, and are considered in the Real-Time Risk and Trust Analysis process, in order to assign a risk value to every potential decision that can be made by the system.
By evaluating the partial solutions proposed by the GA, this approach provides the CSO, who is in charge of assigning and adjusting these values over time, with information about the most dangerous situations for certain assets.
The adaptation or adjustment concerns the change in the value that an asset has due to a loss of importance, for instance, for being and out-to-date document.
