...
\item {\em Invasive or non-invasive data access}. A system can be considered {\em invasive} if a user needs to use the tool to perform their job. % (Paloma) Fergu, te parece bien cambiar esta frase a "A system can be considered {\em invasive} if a user must use a certain tool to perform their job." ? Porque con el "the tool" no me queda muy claro.
...
% Antonio - Fergu, introduce esto, no lo pongas así a pelo. ;D
...
The main feature of KNOX is the use of different containers, or environments, for business and personal sides. Each one includes its own graphical configuration, such as wallpapers, or colour themes, in order to be more easily recognised and distinguished by the user.
...
% Antonio - Fergu, falta por comparar las soluciones de IBM con MUSES (Mobile First y Maas360). Lo pide el revisor... :_(
