...
\begin{itemize}
\item It offers a central deployment, administration and securing of all mobile devices regardless of their OS. %FERGU: securing como palabra no. security control mejor?
...
The use of a server is mainly based in the need of a powerful machine, able to work with massive amounts of data (\textit{Big Data}) \ref{BigData_11}. Thus, part of the high computational cost methods to apply will be derived to this high-performance server, as the mobile devices do not count with enough computational power. These methods will include Data Mining \cite{Cios07DataMining} and Machine Learning \ref{MachineLearning_Bishop06} techniques. This server will also centralise the data gathering from all the devices in the company, composing a big database to which the commented techniques will be applied.
