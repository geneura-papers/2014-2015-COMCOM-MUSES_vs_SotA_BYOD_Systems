...
\author{P. de las Cuevas, J.J. Merelo, A.M. Mora\\P. A. Castillo, P. García-Sánchez, A. Fernández-Ares}
\ead{\{paloma, jmerelo, amorag, pacv, pgarcia, antares\}@geneura.ugr.es}
...
The client is an important part of the a-priori risk treatment, because it both monitors the whole context and finally acts or warns the user in event of a security violation. Security violations are mainly identified by security rules, 
...
which is why MUSES uses DM and CI techniques to improve or optimize them and enhance the coverage. 
That is to say, the more sensors are implemented, the more enforced the security in the device is. 
...
To implement a sensor means to include a programmed method which makes MUSES to start `listening' some parts of a device. 
MUSES implements the following sensors:
...
The dataset is composed by the so-called patterns, which consist of the features of an event 
...
Moreover, as the datasets are huge, because a lot of attributes are extracted from the events so the 
% Antonio - este 'the' no sé si sobra
more complete information is obtained, MUSES applies Big Data processing methods \cite{BigData_11}. %JUSTIFY (Paloma will do it)
...
By this process, the patterns are obtained from the `raw' data, as has been explained. These form what is called a dataset, and from it, 
% Antonio - estaba más correcto como estaba antes (aunque estuviese peor redactado). La primera parte no es pattern mining, es la creación de un dataset, que se ha explicado antes. Por otra parte, 'dataset' es un término conocido en este entorno.
the most frequent or anomalous patterns, can be extracted. The idea is that non-frequent patterns are potentially suspicious, and thus, could be of interest to be checked by the CSO, or to serve as a reference for the rule-refinement process.
...
It consists of extracting the most important variables from the data. Feature selection techniques have been proven \cite{liu1998feature} to preserve the performance of a dataset 
...
On the other hand, 
% Antonio - creo que no se debe poner 'on the other hand' sin tener un 'on the one hand', pero no sé si es una regla estricta.
the refinement or adaptation of the security rules is performed by the \textit{Knowledge Compiler} module, and it uses three EC methods. These methods can be used for inference and optimisation, and they use different data as part of the process, such as:
